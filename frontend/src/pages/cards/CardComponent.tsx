...
export const CardComponent: React.FC<CardComponentProps> = ({ card, handleDelete }) => {
    return (
        <div className={`bg-gradient-to-b from-stone-400 via-stone-600 to-stone-400 w-96 min-h-[537.6px] rounded overflow-hidden shadow-lg relative border-2 border-black rounded-lg`}>
          {/* <button
          ...
          <div className='m-1 mb-0'>
            ...
            { card.type !== "Resource" &&
              <div className="flex item-stretch text-center pt-1.5 h-svh mt-0 text-blue-100">
                <p className="flex item-center justify-center p-4 py-14 bg-neutral-300 center-item text-black h-36 w-full text-base shadow-lg relative border-2 border-black rounded-lg min-h-48 m-0.5 mt-0">
                  Action: {card.action.replace("MOD", card.mod)} <br></br>
                </p>
